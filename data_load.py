...
class RandomRotate(object):
    ...
    def __call__(self, sample):
        ...
        # first transform
        if draw < 0.25:
            return self.cw(self.cw(sample))
        elif draw < 0.5:
            return self.cw(sample)
        elif draw < 0.75:
            return self.ccw(sample)
        else:
            return sample
    ...
    def cw(self, sample) -> "dict[str : np.ndarray]":
        ...
        pts = np.zeros_like(key_pts)
        pts[:, 0], pts[:, 1] = h - key_pts[:, 1], key_pts[:, 0]
        return {"image": img, "keypoints": pts}
    ...
    def ccw(self, sample):
        ...
        pts = np.zeros_like(key_pts)
        pts[:, 0], pts[:, 1] = key_pts[:, 1], w - key_pts[:, 0]
        return {"image": img, "keypoints": pts}
